...
function [alivepreys,alivepredators,predatorgen,preygen]=noquantumsim4(deathvalue,L1,L2,maxstep,probprey,probpredator,initialpreys,initialpredators,maxpreys,maxpredators,gammat,gammaf,anglepredator,angleprey,showplot,saveplot,n,Nsim)
% functions which increase or decrease sz in the way we want
% NEWS: GENOTYPES AND MUTATIONS!
...
% sz (life) value of individuals. Now is a matrix and second column
% represents genotype
szpreys=ones(2,initialpreys)*(sin(angleprey/2)^2-cos(angleprey/2)^2);
szpredators=ones(2,initialpredators)*(sin(anglepredator/2)^2-cos(anglepredator/2)^2);
...
for j=1:initialpreys
    ...
    while isfree==false
        ...
        if isfree
            field(pos(1),pos(2))=1+szpreys(1,j)/2;
            preyspos(j,1)=pos(1);
            preyspos(j,2)=pos(2);
        end
    end
...
for j=1:initialpredators
    ...
    while isfree==false
        ...
        isfree=checkfree(pos,field);
        if isfree
            field(pos(1),pos(2))=2+szpredators(1,j)/2;
...
for step=1:maxstep
        ...
        for j=1:npredators
            if (norm(preyspos(k,:)-predatorspos(j,:))<1.5) && (preyspos(k,1)~=0) && (predatorspos(j,1)~=0)
               ...
               % VERSION 3 DELETES INCREASING OF FEED BY PREDATORS
                szpreys(1,k)=decreasefeed(szpreys(1,k));
                
            end
        end
    end
    
    % pass time AND MUTATIONS (for preys)
    for k=1:npreys
        szpreys(1,k)=decreasetime(szpreys(1,k));
        aux=szpreys(2,k)+(rand-0.5)/25;
        if aux<1
            szpreys(2,k)=aux;
        end
        % this may decrease or increase genotype's value less than +-0.1
    end
    
    % pass time AND MUTATIONS (for predators)
    for k=1:npredators
        szpredators(1,k)=decreasetime(szpredators(1,k));
        aux=szpredators(2,k)+(rand-0.5)/50;
        if aux>deathvalue
            szpredators(2,k)=aux;
        end
    end 
    ...
    for k=1:npreys
        if k<npreys
            if szpreys(1,npreys+1-k)<deathvalue
                szpreys(:,npreys+1-k)=[];
                % remove individual
            ...
            if aux>1 && szpreys(1,1)<deathvalue % the last individual will be killed only if we have another alive individual
                szpreys(:,1)=[];
    ...
    for k=1:npredators
        if k<npredators
            ...
            if aux>1 && szpredators(1,1)<deathvalue
    ...
    for k=1:npreys
        ...
        if (rand<probprey) && (npreys<maxpreys) 
            ...
            % give to new individual as genotypeand phenotype value the value from the father's genotype
            szpreys(1,npreys)=szpreys(2,k);
            szpreys(2,npreys)=szpreys(2,k);
    ...
    for k=1:npredators
        if (rand<probpredator*npreys) && (npredators<maxpredators)
            ...
            % replicate predator using father's phenotype
            szpredators(1,npredators)=szpredators(2,k);
            szpredators(2,npredators)=szpredators(2,k);
    ...
    field=fillfield2(L1,L2,preyspos,predatorspos,szpredators(1,:),szpreys(1,:),deathvalue);
    %alivepredators(step+1)=sum(szpredators>deathvalue);
    %alivepreys(step+1)=sum(szpreys>deathvalue);
    % these previous lines are equivalent but computationally harder
    ...
    %plot([step,step],[0,mean(szpreys(2,:))],'b');
    %hold on
    
    %disp(mean(szpredators(2,:)))
    predatorgen(step)=mean(szpredators(2,:));
    preygen(step)=mean(szpreys(2,:));
end
